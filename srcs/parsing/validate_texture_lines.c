...
void	validate_texture_lines(t_game *game)
{
	...
	game->img_no.buffer = (int *)mlx_get_data_addr(game->img_no.image, &(game->img_no.pixel_bits), &(game->img_no.line_bytes), &(game->img_no.endian));
	game->img_so.buffer = (int *)mlx_get_data_addr(game->img_so.image, &(game->img_so.pixel_bits), &(game->img_so.line_bytes), &(game->img_so.endian));
	game->img_we.buffer = (int *)mlx_get_data_addr(game->img_we.image, &(game->img_we.pixel_bits), &(game->img_we.line_bytes), &(game->img_we.endian));
	game->img_ea.buffer = (int *)mlx_get_data_addr(game->img_ea.image, &(game->img_ea.pixel_bits), &(game->img_ea.line_bytes), &(game->img_ea.endian));
